...
\begin{frame}
    \frametitle{Example: stop sign controller}
    \begin{columns}
        \begin{column}[T]{0.6\textwidth}
        \begin{itemize}
            \item discrete controller for reference position $p_r$
            \item PD-controller for acceleration
                \begin{equation*}
                    \begin{cases}
                        \D{x} = v &\\
                        \D{v} = -K_p(x-p_r)-K_d\,v &
                    \end{cases}
                \end{equation*}
            \item delay in sensing
                \begin{equation*}\small
                    \begin{cases}
                        \D{x} = v &\\
                        \D{v} = -K_p\big(x(t-\tau)-p_r\big)-K_d\,v(t-\tau) &
                    \end{cases}
                \end{equation*}
            \item oscillations
        \end{itemize}
        ...
        \begin{column}[T]{0.4\textwidth}
            ...
            \node [inner sep=0cm, below right] at (0,0)  {
            \includegraphics[width=1.1\textwidth]{tau0-3.png}};
            \node at (3,-1.1) {$x$};
            \node at (3,-6.1) {$v$};
            \node at (3,-3.3) {$\tau=0.3$};
            \end{tikzpicture}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{column}[T]{0.4\textwidth}
            \noindent\begin{tikzpicture}
            \node [inner sep=0cm, below right] at (0,0)  {
            \includegraphics[width=1.1\textwidth]{tau0-4.png}};
            \node at (3,-1.1) {$x$};
            \node at (3,-6.1) {$v$};
            \node at (3,-3.3) {$\tau=0.4$};
            \end{tikzpicture}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{column}[T]{0.4\textwidth}
            \noindent\begin{tikzpicture}
            \node [inner sep=0cm, below right] at (0,0)  {
            \includegraphics[width=1.1\textwidth]{tau0-5.png}};
            \node at (3,-1.1) {$x$};
            \node at (3,-4.6) {$v$};
            \node at (3,-3.3) {$\tau=0.5$};
            \end{tikzpicture}
...
\begin{frame}
    ...
    \begin{definition}[DDE]<1->
        For delay $\tau>0$ and $f\from\R^n\times\R^n\to\R^n$,
        \begin{equation*}
            \D{x}(t) = f(x(t),x(t-\tau))
        \end{equation*}
        is a \emph{delay differential equation} with constant delay.\\
        Can be extended to multiple delays.
    \end{definition}
    \begin{definition}[IVP]<2->
        For an \emph{initial condition} $x_{\tzero}\from \compactum{\tzero-\tau}{\tzero} \to\R^n$, solving
        \begin{equation*}
            \begin{cases}
                \D{x}(t) = f(x(t),x(t-\tau)) & \text{for } t\geq\tzero\\
                x(t) = x_{\tzero}(t) & \text{for } t\in\compactum{\tzero-\tau}{\tzero}
            \end{cases}
        \end{equation*}
...
\begin{frame}
    ...
    \begin{tikzpicture}[line width=0.5pt, scale=\textwidth/9cm, >=Latex]
        ...
        \foreach \y in {-1.0,-0.5,0.0,0.5,1.0} {
            \draw[help lines, dashed] (-1.2,\y) -- (7.2,\y);
            \draw[thick] (-1.2,\y) -- (-1.08,\y);
            \draw[thick] (7.2,\y) -- (7.08,\y);
        }

        \foreach \x in {-1,...,7} {
            \draw[help lines, dashed] (\x,-1.2) -- (\x,1.2);
        ...
        % delay arrow
        \path[->,color=darkbleu303] (-0.5,0.95) edge [bend right] (0.45,0.5);
        \path[->,color=darkbleu303] (2.5,-0.35) edge [bend left] (3.45,0.1);
...
\begin{frame}
    ...
    \begin{columns}[T]
        \begin{column}[T]{0.3\textwidth}
            \begin{tikzpicture}[line width=0.5pt, xscale=\textwidth/2.4cm, yscale=0.9\textheight/7.4cm, >=Latex]
                ...
                % frame
                \draw (-4.2,-3.2) rectangle (-1.8,4.2);


                % grid and ticks
                \foreach \y in {-3.0,-2.0,-1.0,0.0,1.0,2.0,3.0,4.0} {
                    \draw[help lines, color=gray!30, dashed] (-4.2,\y) -- (-1.8,\y);
                    \draw[thick] (-4.2,\y) -- (-4.12,\y);
                    \draw[thick] (-1.8,\y) -- (-1.88,\y);
                }
                \foreach \x in {-4.0,-3.25,-2.0} {
                ...
                \draw (-4.18,0.0) node[right] {$0$};
                

                % initial condition
                \draw (-3.5,1.1) node[above] {$x$};
                \draw (-3.5,-1.5) node[below] {$\D{x}$};
        ...
        \begin{column}[T]{0.7\textwidth}
            \begin{definition}[Piecewise Cont. Differentiable]
            For a partition $\partition[t_1]{a=t_0}{t_p=b}$,
            \begin{equation*}
                x\from \compactum{a}{b}\to\R^n
            \end{equation*}
            is $m$-times \emph{piecewise continuously differentiable} iff for all $k=\range{0}{m}$:
            \begin{enumerate}
...
\begin{frame}
    ...
    \begin{theorem}[Existence of a unique solution]
        Let $f\from\R^n\times\R^n\to\R^n$ continuous and Lipschitz in its first argument and $x_{\tzero}$ \emph{piecewise continuous},\\
        then there \emph{exists} a \emph{unique local solution} of the IVP on a time interval $\compactum{\tzero-\tau}{\tzero+T}$.
    \end{theorem}
...
\begin{frame}
    ...
    \tikzstyle{na} = [baseline=-1.75ex]
    \tikzstyle{nb} = [baseline=0.6ex]
    \tikzstyle{nc} = [baseline=-0.5ex]
    \frametitle{Towards Logics}
    Example DDE: show $x(t)\leq 1$ for all $t\geq 0$ \tikz[nc]{\node [coordinate] (n3) {};}
    \begin{equation*}
        \begin{cases}
            \D{x}(t) = -x(t-1) \tikz[nb]{ \node [coordinate] (n1) {};}
                & t\geq 0\\
    ...
    Notation in \ddL
    \begin{equation*}
        \hsc[-1]{\tikz[na]\node [coordinate] (t2) {}; \x[s]=1}
        \limply \dbox{\hevolve{\D{x}=\tikz[na]{\node [coordinate] (t1) {};}\x[-1]}}{(x\leq 1)}\tikz[nc]{\node [coordinate] (t3) {};}
    \end{equation*}

    \begin{tikzpicture}[overlay]
        \path[->,color=darkbleu303] (n1) edge [out=-90, in=120] (t1);
        \path[->,color=darkbleu303] (n2) edge [out=-90, in=90] (t2);
        \path[->,color=darkbleu303] (n3) edge [out=0, in=0] (t3);
    \end{tikzpicture}
\end{frame}
...
\begin{frame}
    ...
    \begin{tikzpicture}[font=\footnotesize,line width=0.5pt, xscale=\textwidth/8.4cm, yscale=0.5\textheight/7.4cm, >=Latex]
        ...
        % sliding window
        \draw[dashed,color=vert575,fill=ultralightvert575] (-4,-2.4) rectangle (0,3.4);
        \draw[dashed,color=bleu303,fill=bleu303,fill opacity=0.2] (-2,-2.4) rectangle (2,3.4);

        \draw[|-|] (-4,-2.8) -- (0,-2.8) node [midway, below] {state at $t=0$};
        \draw[|-|] (-2,3.8) -- (2,3.8) node [midway, above] {state at $t=\zeta$};

        % frame
        \draw (-4.2,-2.4) rectangle (4.2,3.4);

        % grid and ticks
        \foreach \y in {-2.0,-1.0,0.0,1.0,2.0,3.0} {
            % \draw[help lines, dashed] (-4.2,\y) -- (4.2,\y);
            \draw[thick] (-4.2,\y) -- (-4.15,\y);
            \draw[thick] (4.2,\y) -- (4.15,\y);
        }
        \foreach \x in {-4.0,-3.25,-2.0,-1.2,0.0,1.0,2.0,3.0,4.0} {
            % \draw[help lines, dashed] (\x,-3.2) -- (\x,4.2);
            \draw[thick] (\x,3.4) -- (\x,3.2);
            \draw[thick] (\x,-2.4) -- (\x,-2.2);
        }

        \draw (-4.0,-2.2)   node[above] {$-\tau$};
        \draw (0,-2.2)  node[above] {$0$};
        \draw (2.0,-2.2)   node[above] {$\zeta$};
        \draw (-2,-2.2)   node[above] {$\zeta-\tau$};

        \draw (2.57,0.8) node {$\D{x}=-x(t-\tau)$};
...
\begin{frame}
    \frametitle{Semantics}
    \begin{itemize}
        \item valuation of s-terms and s-formulas
        \item depends on assignment $\past\in\closeddelayinterval$ to $s$
        \item Example \begin{equation*}
...
\begin{frame}
    ...
    \begin{columns}[T]
        \begin{column}[T]{0.6\textwidth}
            \begin{itemize}
                \item Assignment only changes present value, not past!
                \begin{equation*}
                ...
                \end{equation*}
                \item Test condition over entire state possible: $\bsfml\equiv\hs{\tilde{\bsfml}(s)}$
                \begin{equation*}
        ...
        \end{column}
        \begin{column}[T]{0.4\textwidth}
            \begin{tikzpicture}[line width=0.5pt, xscale=(\textwidth-20pt)/1.4cm, yscale=0.6\textheight/3.7cm, >=Latex]

                \newcommand{\polythree}{\ca+\cb*((\x-\ta)/(\tb-\ta))+\cc*((\x-\ta)/(\tb-\ta))^2+\cd*((\x-\ta)/(\tb-\ta))^3}
                
                % frame
                \draw[->] (-1.1,-1.5) -- (-1.1,1.8) node[above] {$\x[s]$};
                \draw[->] (-1.1,-1.5) -- (0.2,-1.5) node[right] {$s$};

                \draw (-1,-1.5) node {$[$};
                \draw (0,-1.5) node {$)$};
                \draw (-1,-1.65) node[below] {$-T$};
                \draw (0,-1.65) node[below] {$0$};

                \draw[leftp] (0,0.1) -- (0,0.1);
...
\begin{frame}
    ...
    \begin{itemize}
        \item derived \emph{delay differential induction} axiom
        \item for s-quantified safety condition
    \end{itemize}
...
\begin{frame}
    \frametitle{Outlook}
    \begin{itemize}
        \item<1-> Example proofs!
        \item<2-> How to find differential invariants?
